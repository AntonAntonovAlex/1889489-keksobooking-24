...
const getArrayElements = (array) => {
  ...
  for (let i = 0; i < randomLength; i++) {
    const randomInteger = getRandomNumber(0, cloneArray.length-1);
...
const createAnnouncements = (numberAnnouncements) => {
  const newAnnouncements = [];
  for (let i = 0; i < numberAnnouncements; i++) {
    newAnnouncements[i] = createAnnouncement(i+1);
  }
  return newAnnouncements;
};

createAnnouncements(NUMBER_ANNOUNCEMENT);
